...
class CLIConfig(BaseModel):
    ...
    profiles: Dict[str, MISPProfile] = Field(
        default_factory=dict, description="MISP profiles"
    )
    
    def get_profile(self, name: Optional[str] = None) -> MISPProfile:
    ...
    def _parse_config(self, config_path: Path) -> Dict[str, Any]:
        ...
        for section in parser.sections():
            if section.startswith("profile:"):
                ...
                profile_data["colorize"] = self._parse_bool(
                    section_dict.get("colorize", "true")
                )
                
                # Integer values
                try:
                    profile_data["timeout"] = int(
                        section_dict.get("timeout", "30")
                    )
                except ValueError:
                    profile_data["timeout"] = 30
                
                # String values
                profile_data["output_format"] = section_dict.get(
                    "output_format", "json"
                ).strip()
        ...
        if "default" in parser and not any(
            s.startswith("profile:") for s in parser.sections()
        ):
    ...
    def load(self) -> CLIConfig:
        ...
        if self.config_path is None or not self.config_path.exists():
            raise FileNotFoundError(
                ...
                f"MISP_CLI_CONFIG, ~/.misp-cli.conf, ./.misp-cli.conf"
            )
...
class MISPConfig:
    ...
    @classmethod
    def from_file(cls, config_path: Optional[Path] = None) -> "MISPConfig":
        """Load configuration from file.
