...
def list_blocklist(
    ...
    entries = response.get("attributeBlocklists", response.get("data", []))
...
def add_to_blocklist(
    ...
    attr_type: Optional[str] = typer.Option(None, "-t", "--type", help="Attribute type"),
    comment: Optional[str] = typer.Option(None, "-c", "--comment", help="Comment"),
    json_output: bool = typer.Option(False, "--json", help="Output as JSON"),
):
    """Add an attribute to the blocklist."""
    from misp_cli.cli.app import get_app
    ...
    if config.output_format == "json" or json_output:
        ...
        entry_id = response.get("AttributeBlocklist", {}).get("id", "Unknown")
        typer.echo(f"Value added to blocklist: {entry_id}")


@attribute_blocklists_app.command("delete")
def remove_from_blocklist(
    ...
    response = client.post_sync(f"/attributeBlocklists/delete/{entry_id}")
...
@attribute_blocklists_app.command("check")
def check_attribute(
    value: str = typer.Argument(..., help="Value to check"),
    json_output: bool = typer.Option(False, "--json", help="Output as JSON"),
):
    """Check if a value is blocklisted."""
    from misp_cli.cli.app import get_app
    ...
    response = client.get_sync("/attributeBlocklists/check", params={"value": value})
    
    if config.output_format == "json" or json_output:
        _print_json(response)
    else:
        result = response.get("AttributeBlocklist", {})
        if result.get("blocked"):
            typer.echo(f"Value is BLOCKED: {result.get('comment', '')}")
        else:
            typer.echo("Value is not blocklisted")
