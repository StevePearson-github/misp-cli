...
def show_organisation(
    ...
    if output_format == "table":
        _print_table([org_data])
    else:
        _print_json(org_data)


@organisations_app.command("create")
def create_organisation(
    name: str = typer.Option(..., "-n", "--name", help="Organisation name"),
    description: Optional[str] = typer.Option(None, "-d", "--description", help="Organisation description"),
    type: Optional[str] = typer.Option(None, "-t", "--type", help="Organisation type (e.g., ADMIN, Commercial, Educational)"),
    nationality: Optional[str] = typer.Option(None, "--nationality", help="Organisation nationality"),
    sector: Optional[str] = typer.Option(None, "-s", "--sector", help="Organisation sector"),
    contacts: Optional[str] = typer.Option(None, "-c", "--contacts", help="Organisation contacts"),
    local: Optional[bool] = typer.Option(None, "--local", help="Whether the organisation is local"),
    uuid: Optional[str] = typer.Option(None, "--uuid", help="Organisation UUID"),
    restricted_to_domain: Optional[str] = typer.Option(None, "--restricted-to-domain", help="Comma-separated list of domains to restrict to"),
    json_output: bool = typer.Option(False, "--json", help="Output as JSON"),
):
...
def edit_organisation(
    ...
    name: Optional[str] = typer.Option(None, "-n", "--name", help="New organisation name"),
    description: Optional[str] = typer.Option(None, "-d", "--description", help="New organisation description"),
    type: Optional[str] = typer.Option(None, "-t", "--type", help="New organisation type (e.g., ADMIN, Commercial, Educational)"),
    nationality: Optional[str] = typer.Option(None, "--nationality", help="New organisation nationality"),
    sector: Optional[str] = typer.Option(None, "-s", "--sector", help="New organisation sector"),
    contacts: Optional[str] = typer.Option(None, "-c", "--contacts", help="New organisation contacts"),
    local: Optional[bool] = typer.Option(None, "--local", help="Whether the organisation is local"),
    uuid: Optional[str] = typer.Option(None, "--uuid", help="Organisation UUID"),
    restricted_to_domain: Optional[str] = typer.Option(None, "--restricted-to-domain", help="Comma-separated list of domains to restrict to"),
    json_output: bool = typer.Option(False, "--json", help="Output as JSON"),
    ...
    if restricted_to_domain is not None:
        # Convert comma-separated string to list
        data["restricted_to_domain"] = [d.strip() for d in restricted_to_domain.split(",")] if restricted_to_domain else []
